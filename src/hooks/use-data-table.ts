...
export function useDataTable<TData>(props: UseDataTableProps<TData>) {
  ...
  const [perPage, setPerPage] = useQueryState(SIZE_KEY, parseAsInteger.withOptions(queryStateOptions).withDefault(10));
  ...
  // Parse sort from URL (format: "columnId,direction")
  const [sortParam, setSortParam] = useQueryState(SORT_KEY, parseAsString.withOptions(queryStateOptions).withDefault(""));

  // Convert URL param to sorting state
  ...
  const onSortingChange = React.useCallback(
    (updaterOrValue: Updater<SortingState>) => {
      ...
      if (newSorting.length === 0) {
        ...
        // Update sorting (format: "columnId,direction")
        const { id, desc } = newSorting[0];
        void setSortParam(`${id},${desc ? "desc" : "asc"}`);
      }
    },
