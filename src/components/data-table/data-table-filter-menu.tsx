...
export function DataTableFilterMenu<TData>({ table, debounceMs = DEBOUNCE_MS, throttleMs = THROTTLE_MS, shallow = true, align = "start", ...props }: DataTableFilterMenuProps<TData>) {
  ...
  return (
    <div className="flex flex-wrap items-center gap-2">
      {filters.map((filter) => (
        <DataTableFilterItem key={filter.filterId} filter={filter} filterItemId={`${id}-filter-${filter.filterId}`} columns={columns} onFilterUpdate={onFilterUpdate} onFilterRemove={onFilterRemove} />
      ))}
      {filters.length > 0 && (
        <Button aria-label="Reset all filters" variant="outline" size="icon" className="size-8" onClick={onFiltersReset}>
          <X />
        </Button>
      )}
      <Popover open={open} onOpenChange={onOpenChange}>
        ...
        <PopoverContent align={align} className="w-full max-w-[var(--radix-popover-content-available-width)] origin-[var(--radix-popover-content-transform-origin)] p-0" {...props}>
...
function DataTableFilterItem<TData>({ filter, filterItemId, columns, onFilterUpdate, onFilterRemove }: DataTableFilterItemProps<TData>) {
  {
    ...
    return (
      <div key={filter.filterId} role="listitem" id={filterItemId} className="flex h-8 items-center rounded-md bg-background" onKeyDown={onItemKeyDown}>
...
function FilterValueSelector<TData>({ column, value, onSelect }: FilterValueSelectorProps<TData>) {
  ...
  switch (variant) {
    ...
    case "date":
    case "dateRange":
      return <Calendar initialFocus mode="single" selected={value ? new Date(value) : undefined} onSelect={(date) => onSelect(date?.getTime().toString() ?? "")} />;
            ...
            <Button id={inputId} aria-controls={inputListboxId} variant="ghost" size="sm" className="h-full min-w-16 rounded-none border px-1.5 font-normal dark:bg-input/30">
              {selectedOptions.length === 0 ? (
                ...
                <>
                  ...
                  <span className="truncate">{selectedOptions.length > 1 ? `${selectedOptions.length} selected` : selectedOptions[0]?.label}</span>
                </>
              )}
            </Button>
          </PopoverTrigger>
          <PopoverContent id={inputListboxId} align="start" className="w-48 origin-[var(--radix-popover-content-transform-origin)] p-0">
            <Command>
                ...
                <CommandGroup>
                  {options.map((option) => (
                    ...
                    >
                      {option.icon && <option.icon />}
                      <span className="truncate">{option.label}</span>
                      {filter.variant === "multiSelect" && <Check className={cn("ml-auto", selectedValues.includes(option.value) ? "opacity-100" : "opacity-0")} />}
                    </CommandItem>
                  ))}
                </CommandGroup>
              </CommandList>
    ...
    case "dateRange": {
      ...
      const displayValue =
        filter.operator === "isBetween" && dateValue.length === 2
          ? `${formatterService.formatDate(new Date(Number(dateValue[0])))} - ${formatterService.formatDate(new Date(Number(dateValue[1])))}`
          : dateValue[0]
          ? formatterService.formatDate(new Date(Number(dateValue[0])))
          : "Pick date...";
        ...
        <Popover open={showValueSelector} onOpenChange={setShowValueSelector}>
          <PopoverTrigger asChild>
            <Button
              ...
              variant="ghost"
              size="sm"
              className={cn("h-full rounded-none border px-1.5 font-normal dark:bg-input/30", !filter.value && "text-muted-foreground")}
            >
