...
import { searchShowTypesParamsCache } from "./validation";

export function ShowTypesTable() {
  const t = useTranslations();

  const params = getAllParams(useSearchParams());
  const searchParams = searchShowTypesParamsCache.parse(params);
  const filteredSearch = getValidSearchParams(searchParams);

  const { data, isLoading, error } = useQuery({
    queryKey: ["show-types", filteredSearch],
    queryFn: () => filterService.getShowTypes(filteredSearch),
